					...
					<div key={item.id} className="relative transition duration-300 hover:scale-110 cursor-pointer">
						...
						<div className="absolute bottom-0 bg-gradient-to-t from-black via-black/80 to-black/30 h-full w-full flex flex-col gap-4 py-3 px-6 justify-end items-start">
							<div>
								...
								<p className="font-light text-sm text-neutral-400">Released on {item.released}</p>
							</div>
							<Link to={`/games/${item.id}`} className="w-full">
								<button className="bg-transparent border border-gray-500 py-2  w-full transition duration-100 active:scale-95 whitespace-nowrap hover:bg-neutral-700/20">
